...
async def _run_agents_in_background(report_id: str, token_id: str, report_repository: ReportRepository):
    try:
        await report_repository.update_report_status(report_id, ReportStatusEnum.PROCESSING)
...
async def generate_report_endpoint(request: ReportRequest, background_tasks: BackgroundTasks, session: AsyncSession = Depends(get_session)):
    ...
    background_tasks.add_task(_run_agents_in_background, report_id, request.token_id, report_repository)
